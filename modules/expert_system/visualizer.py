...
def _render_kbtb_block(questions):
    ...
    st.markdown("### ⚖️ Коэффициент сбалансированности тестовой базы (KBTB)")
    ...
    st.metric("KBTB", f"{kbtb * 100:.1f}%", f"Интерпретация: {interp}")
    # Разбивка штрафов
    st.markdown("**Влияние на коэффициент:**")
    if p_type > 0.001:
        st.caption(f"−{p_type * 100:.1f}% из-за перекоса по типам вопросов (O/Z)")
    if p_level > 0.001:
        st.caption(f"−{p_level * 100:.1f}% из-за перекоса по сложности (L/M/H)")
    if p_rework > 0.001:
        st.caption(f"−{p_rework * 100:.1f}% из-за вопросов на переделку")
    if p_count > 0.001:
    ...
    cats = ['O (открытые)', 'Z (закрытые)', 'L (лёгкие)', 'M (средние)', 'H (сложные)']
    target_pct = [
    ...
    st.caption(f"Всего вопросов: {res['n']}, из них «на переделку»: {res['n_rework']} ({res['R']*100:.1f}%).")
    ...
    with st.expander("📖 Справка: Формула KBTB"):
        ...
        **Штрафы:**
        - $P_{\\text{переделка}} = 1 - e^{-3R}$ — штраф за вопросы на переделку
