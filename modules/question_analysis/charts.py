...
def create_difficulty_distribution_plot(questions_data: List[Dict[str, Any]], question_type_filter: str = None):
    ...
    if marked_categories:
        ann = [dict(
            text="Позиция вопроса в тесте. Пропуск столбца — повторное задействование категории",
    ...
    fig_left.add_trace(go.Bar(
        ...
        hovertemplate='<b>Позиция вопроса %{x}</b><br>Количество вопросов: %{y}<extra></extra>'
    ))
    fig_left.update_layout(
        title=dict(
            text='Количество вопросов в категории банка заданий,<br>используемых на позиции в тесте',
        ...
        height=500,
        showlegend=False,
        margin=dict(l=50, r=30, t=80, b=95),
        annotations=ann
    )
    ...
    fig_right.update_layout(
        title=dict(
            text='Средние показатели по категориям банка заданий,<br>используемых на позиции в тесте',
            x=0.5, xanchor='center',
            font=dict(size=13)
        ),
        xaxis=dict(title='', type='category'),
        ...
        margin=dict(l=50, r=30, t=80, b=130),
        annotations=ann
    )
